...
pub fn kernel(
    ...
    for delta in deltas {
        ...
        let mut elevation_delta = elevations[dem_id] - pov_elevation;
        let mut distance_delta = (distances[dem_id] - pov_distance).abs();

        // TODO: Is there a better way to do this? Perhaps by ensuring this
        //       never happens in the first place?
        if distance_delta.is_infinite() {
            distance_delta = 0.0;
        }
        if elevation_delta.is_infinite() {
            elevation_delta = 0.0;
        }
