...
double *DEM::readHeights() {
  ...
  if(f == NULL)
  {
    ...
    for(int i = 0; i < DEM_SIZE; i++)
    {
      ...
      // It's like the DEM is a piece of paper and you flip the top-right corner to
      // the bottom-left. But why!?
      //h[i/xdim+ydim*(i%xdim)] = num/SCALE; //internal representation from top to row

      // Why divide by SCALE? Is it multiplied in again later for the total viewshed surface area calculation?
...
void DEM::compute(bool is_precomputing) {
  ...
  if(!is_precomputing) {
    ...
    for (int point = 0; point < this->size; point++) this->cumulative_surface[point] = 0;
  ...
  for(int sector_angle = 0; sector_angle < TOTAL_SECTORS; sector_angle++) {
    ...
    if(!is_precomputing) {
      ...
      for (int point = 0; point < this->size; point++) {
        //if(this->sector->surfaceB[point] > 100) LOGI << point << ":" << sector_angle << ":" << sector->surfaceB[point];
        this->cumulative_surface[point] += this->sector.surfaceF[point] + this->sector.surfaceB[point];
      }
    }
  }
