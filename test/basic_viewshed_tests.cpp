...
// Here we use a simple ASCII representation of the opening and closing of ring sectors.
// A '.' can be either inside or outisde a ring sector.
// A '+' is the opening and a '-' is the closing.
// Eg; Given this ASCII art profile of 2 mountain peaks, where the observer is 'X':
//
//                     .*`*.
//                  .*`     `*.
//       .      X.*`           `*.
...
SCENARIO("Basic viewsheds") {
  GIVEN("A DEM with a symmetrical mountain summit in the middle") {
    ...
    THEN("the mountain should prevent a viewer in a corner seeing the far corner") {
      std::string expected_viewshed =
        "+ . . . . \n"
        ". . - - - \n"
        ". - . - . \n"
  ...
  GIVEN("A double peaked DEM") {
    ...
    THEN("a viewer in the top-left corner should see 2 ring sectors") {
      ...
      // Maybe because ring sectors aren't exactly the same concept as visible points?
      std::string expected_viewshed =
        "+ . - . . \n"
        ". . - . . \n"
        "- - - + - \n"
